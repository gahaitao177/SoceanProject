...
/**
  ...
  */
object AppKafkaSparkStatsGaoht {
  def main(args: Array[String]): Unit = {
    ...
    //小时，天级别活跃用户的统计
    activeUserCountHour(jsonDStream, scc)

    //天级别活跃用户的统计
    activeUserCountDay(jsonDStream, scc)
  ...
  /**
    ...
    * @param scc
    */
  def activeUserCountHour(reports: DStream[JSONObject], scc: StreamingContext) = {
    ...
    val broadcastUserHourMap = scc.sparkContext.broadcast(activeUserMap)
  ...
  def activeUserCountDay(reports: DStream[JSONObject], scc: StreamingContext) = {
    val active_user_stat_day = ConfigurationManager.getProperty(Constants.HBASE_APP_DATA_DAILY)
    val broadcastActiveUserStat = scc.sparkContext.broadcast(active_user_stat_day)

    val activeUserMap = mutable.Map[String, mutable.Map[String, Int]]()

    val broadcastUserDayMap = scc.sparkContext.broadcast(activeUserMap)
    ...
    reports.foreachRDD(rdd => {
      rdd.foreachPartition(partitionOfRecords => {
        ...
        partitionOfRecords.foreach(x => {
          ...
          try {
            //判断当前rowKey在内存中是否存在
            val flag = broadcastUserDictMap.value.contains(userDictRowKey)
            ...
            if (!flag) {
              ...
              if (isNotExistKey) {
                ...
                //更新内存中不存在的该rowKey
                broadcastUserDictMap.value += (userDictRowKey -> 1)
              } else {
                //更新内存中不存在的该rowKey
                broadcastUserDictMap.value += (userDictRowKey -> 1)
              }
      ...
      rdd.foreachPartition(partitionOfRecords => {
        ...
        partitionOfRecords.foreach(x => {
          ...
          try {
            ...
            val flag = broadcastUserInfoMap.value.contains(userInfoRowKey)
            ...
            if (!flag) {
              ...
              if (isNotExistKey) {
                ...
                //更新内存中不存在的该rowKey
                broadcastUserInfoMap.value += (userInfoRowKey -> 1)
              } else {
                //更新内存中不存在的该rowKey
                broadcastUserInfoMap.value += (userInfoRowKey -> 1)
              }
